...
- Documented and tested that `@startuml`/`@enduml` are optional: when the diagram body omits them, PlantUML adds them automatically, so the `plantuml` environment may contain just the diagram. Added `example-without-startuml.tex`. [#4](https://github.com/koppor/plantuml/issues/4)
